@extends('layouts.app', ['title' => __('Lowongan Kerja')]) @section('content')
          ...
          <form method="post" action="{{ route('job.update', $job) }}" autocomplete="off" enctype="multipart/form-data">
            ...
            <div class="pl-lg-4">
              <div class="form-group">
                ...
                <div class="card">
                  <div class="card-body" style="max-height: 10rem; overflow: auto">
                    <p id="raw-data" style="white-space: pre-wrap">
                      {{ $job->raw_data }}
                    </p>
                  </div>

                  <div class="card-footer">
                    <button id="copy-raw-data" class="btn btn-sm btn-icon btn-3 btn-primary" type="button">
                      <span class="btn-inner--icon"><i class="fa fa-copy"></i></span>
                      <span class="btn-inner--text">Salin data mentah</span>
                    </button>
                    <small id="copy-raw-data-status"  style="display: none"></small>
                  </div>
                </div>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <img id="img-poster" class="poster-preview w-100 mb-3 d-none" src="" alt="Poster / Pamflet"
                  data-toggle="modal" data-target="#zoomImageModal" style="height: 256px; object-fit: contain" />
                <div class="row mx-0 mb-3" style="flex-wrap: nowrap;">
                  ...
                  <button id="poster-add" type="button" class="btn btn-primary">
                    Tambah Gambar
                  </button>
                  <button id="poster-remove" type="button" class="btn btn-danger d-none">
                    Hapus Gambar
                  </button>
                </div>
              ...
              <div class="form-group">
                ...
                <input type="text" name="position" id="input-position"
                  class="form-control{{ $errors->has('position') ? ' is-invalid' : '' }}" placeholder="Posisi"
              ...
              <div class="form-group">
                ...
                <select type="text" name="province" id="input-province"
                  class="form-control {{ $errors->has('province') ? ' is-invalid' : '' }}" placeholder="Provinsi">
                </select>
              ...
              <div class="form-group">
                ...
                <input type="text" name="email" id="input-email"
                  class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" placeholder="Email (opsional)"
              ...
              <div class="form-group">
                <p class="form-control-label" for="input-requirements">
                  Persyaratan
                </p>
                ...
                <div class="row mx-0 mb-3" style="flex-wrap: nowrap;">
                  ...
                  <button type="button" class="btn-remove btn btn-danger ml-2">
                    <i class="fa fa-minus"></i>
                  </button>
                </div>
                @else @foreach ($requirements as $value)
                <div class="row mx-0 mb-3" style="flex-wrap: nowrap;">
...
<script type="text/javascript">
  $(function () {
    ...
    $('#copy-raw-data').click(function () {
      try {
        navigator.clipboard.writeText($('#raw-data').text()).then(() => {
          $('#copy-raw-data-status').show();
          $('#copy-raw-data-status').text('Data mentah tersalin');
          setTimeout(() => {
            $('#copy-raw-data-status').fadeOut('slow');
          }, 1000);
        });
      } catch (error) {
