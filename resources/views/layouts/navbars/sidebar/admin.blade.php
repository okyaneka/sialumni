    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="sidenav-collapse-main">
            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a href="#alumni" class="nav-link {{ Request::is('user*') ? 'active' : '' }}" data-toggle="collapse"
                        role="button" aria-expanded="true">
                        <i class="ni ni-hat-3 text-orange"></i> {{ __('Alumni') }}
                    </a>

                    <div id="alumni"
                        class="collapse bg-secondary {{ Request::is('user*') || Request::is('batch/user') ? 'show' : '' }}">
                ...
                <li class="nav-item">
                    @php $pending = \App\Job::PendingJob()->count(); @endphp
                    <a href="#job" class="nav-link {{ Request::is('job*') ? 'active' : '' }}" data-toggle="collapse"
                        role="button" aria-expanded="true">
                        <i class="fas fa-briefcase text-pink"></i> Lowongan Kerja
                        @if (!empty($pending))
                        <span class="ml-2 badge badge-primary">{{ $pending }}</span>
                        @endif
                    </a>
                    ...
                    <div id="job" class="collapse bg-secondary {{ Request::is('job*') ? 'show' : '' }}">
                        <ul class="nav nav-sm flex-column">
                            <li class="nav-item {{ Request::is('job/index') ? 'active' : '' }}">
                                <a href="{{ route('job.index') }}" class="nav-link">Daftar lowongan kerja</a>
                            </li>
                            <li class="nav-item {{ Request::is('job/pending') ? 'active' : '' }}">
                                <a href="{{ route('job.pending') }}" class="nav-link">
                                    Lowongan kerja pending
                                    @if (!empty($pending))
                                    <span class="ml-2 badge badge-primary">{{ $pending }}</span>
                                    @endif
                                </a>
                            ...
                            <li class="nav-item {{ Request::is('job/create') ? 'active' : '' }}">
                                <a href="{{ route('job.create') }}" class="nav-link">Buat lowongan kerja</a>
                            </li>
                        </ul>
                    </div>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('statistic*') ? 'active' : '' }}" href="#statistic"
                ...
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('department*') || Request::is('status*') || Request::is('group*') ? 'active' : '' }}"
                        href="#master-data" data-toggle="collapse" role="button" aria-expanded="true"
                        aria-controls="master-data">
                        <i class="ni ni-folder-17 text-blue"></i> {{ __('Master Data') }}
                    </a>

                    <div class="collapse {{ Request::is('department*') || Request::is('status*') || Request::is('group*') ? 'show' : '' }} bg-secondary"
                        id="master-data">
                        <ul class="nav nav-sm flex-column">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ Request::is('data*') ? 'active' : '' }}" href="{{ route('larecipe.index') }}"
                        target="_blank">
